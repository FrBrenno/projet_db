SELECT decade, MAX(most_consumed_medication) AS most_frequent_medication
FROM (
    SELECT 
    ...
    FROM patients t1
    JOIN (
        SELECT p.date_de_naissance, dp.DCI, COUNT(*) AS medication_count
        FROM dossiers_patients dp, patients p 
        WHERE dp.NISS_patient = p.NISS
        GROUP BY p.date_de_naissance, dp.DCI
